...
awful.key({                   }, "F12", function ()
    scratch.drop("xfce4-terminal", "top", "center", 1, 0.33) end,
            {description = "open a dropdown terminal", group = "launcher"}),

-- change backlight
awful.key({ modkey,           }, "k", function ()
    awful.spawn("sudo toggle_kb_brightness.py") end,
            {description = "change the keyboard brightness", group = ""}),
...
awful.spawn("/home/daniel/.screenlayout/home.sh")
awful.util.spawn("nm-applet")
awful.util.spawn("sudo toggle_kb_brightness.py off")
awful.util.spawn("sudo fan_control_daemon.sh")
...
-- input for fan control
beautiful.get().fan:connect_signal("button::press", function(c, x)
    awful.prompt.run {
        prompt       = "Fan control: ",
        textbox      = awful.screen.focused().mypromptbox.widget,
        exe_callback = function(input)
            ...
            fan_control_pid = capture("cat /var/run/fan_control.pid")

            -- TODO
            cmd = "/usr/bin/fan_control "
            cmd = cmd .. fan_control_pid .. " "
            ...
            -- p <value> or d <value>
            -- indices start at 1
            t_or_d = input:sub(1, 1)
            value = input:sub(3)

            if t_or_d == 't' then -- set desired temperature
                cmd = cmd .. "set_temp_" .. cpu_or_gpu .. " " .. value
            elseif t_or_d == 'd' then -- set fixed duty cycle
                cmd = cmd .. "set_duty_" .. cpu_or_gpu .. " " .. value
            end
